...
<?php 
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES["files"]["error"] > 0){
      $filenameErr="Return Code: " . $_FILES["files"]["error"] . "<br />";
      echo "<script>alert('{$filenameErr}');history.back();</script>";
      exit(1);
      } else {;
    ...
    if (file_exists("./MIBPBRun/$JobID/" . $_FILES["files"]["name"]))
      {
        ...
        if (file_exists("$CWDir/MIBPBRun/$JobID/$JobID.log") && !file_exists("$CWDir/MIBPBRun/$JobID/Result_$JobID.zip")){
          echo "Your job $JobID is still running, please wait and use this website address to take your result later. <br/> You will also receive an email when the job finish.";
          exit(1);
        }
      }
    else
      {
      //echo "<script>window.location='http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."?software=mibpb&JobID=".$JobID."'</script>;location.reload();";
...
<div style="margin:auto;margin-right:30px;float:left;">
    <form enctype="multipart/form-data" action=<?php echo "submit.php?software=mibpb&JobID={$JobID}"; ?> method="POST" >

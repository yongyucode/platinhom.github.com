...
re模块是Python正则表达式模块, 能够对字符串进行模式匹配, 抓取内容, 十分常用. 主要有两个类: **RegexObject** 类和 **Match** 类, 前者是对象化的正则表达式, 后者是匹配和匹配结果对象化.
...
以上匹配/搜索还可以简单化为: `re.match/search(pattern, string[,flags])` 返回match对象. 这种使用re函数方法适用于简单进行匹配, 不需要重复使用匹配pattern (因为没有返回pattern). 
...
使用pattern进行从头匹配字符串string.
- `re.search(pattern, string[,flags])` :  
使用pattern进行搜素匹配字符串string.

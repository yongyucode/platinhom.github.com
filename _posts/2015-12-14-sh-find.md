...
表达式选项是用来判断递归文件夹文件时, 将文件和表达式进行比较, 符合的返回True, 否则返回False. 返回True的会被进一步被操作. 支持一些判断式操作符, 例如`(), !, -not, -true, -false, -and, -or`. 默认多条表达式是and的关系. `!`使用要前后空格形成独立参数, 并且加反义符`\!`.
